...
class CustomJWTAuthentication(JWTAuthentication):
    ...
    - Creates role-specific profile (Vendor, Customer, Admin)
    """
            ...
            with transaction.atomic():
                ...
                if not user:
                    ...
                    if user:
                        ...
                        logger.warning(f"Conflict found in Ecommerce: User '{username}' exists with ID {user.id}. Updating to {user_id}")
                ...
                # 🔹 Sync Profiles
                if role:
                    from ecommerce.models import AdminProfile, VendorProfile, CustomerProfile
                    if role == "admin":
                        AdminProfile.objects.get_or_create(user=user, defaults={"id": user.id, "full_name": full_name})
                    elif role in ["vendor", "ecommerce_vendor"]:
